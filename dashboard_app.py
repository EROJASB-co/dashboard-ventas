...
with col_a:
    ...
    if not df.empty:
        st.plotly_chart(px.bar(df, x='mes', y='total', color_discrete_sequence=['
#3b82f6']), width='stretch')
    else:
        st.info("Sin datos")
with col_b:
    ...
    if not df.empty:
        st.plotly_chart(px.bar(df, x='total', y='marca', orientation='h', color_discrete_sequence=['
#10b981']), width='stretch')
    else:
        st.info("Sin datos")
...
with col_c:
    ...
    if not df.empty:
        st.plotly_chart(px.bar(df, x='total', y='nombre_vendedor', orientation='h', color_discrete_sequence=['
#f59e0b']), width='stretch')
    else:
        st.info("Sin datos")
with col_d:
...
if not df.empty:
    st.plotly_chart(px.line(df, x='anio', y='total', markers=True, color_discrete_sequence=['
#8b5cf6']), width='stretch')
else:
    st.info("Sin datos")
